...
interface KanbanCard { name: string; role: string; company: string; score: number; date: string }
...
export function ApplicationTracking() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/applications/kanban`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then(r => r.json())
      .then(data => setKanban(data))
      .catch(() => setKanban({}))
        ...
        <>
          ...
          {totals === 0 ? (
            <div className="text-center py-12 rounded-xl bg-white" style={{ border: '1px solid #e2e8f0' }}>
              <p className="text-slate-400 text-sm">No applications yet. Start applying to jobs!</p>
            </div>
          ) : (
            <div className="grid grid-cols-5 gap-3">
              {stages.map(stage => {
